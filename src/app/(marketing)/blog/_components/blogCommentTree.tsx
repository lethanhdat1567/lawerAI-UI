...
import {
  blogMeCommentLikesBatch,
  blogMeCreateComment,
  blogMeDeleteComment,
  blogMePatchComment,
  blogMeToggleCommentLike,
...
function collectBlogCommentIds(nodes: BlogCommentNode[]): string[] {
  const out: string[] = [];
  function walk(ns: BlogCommentNode[]) {
    for (const n of ns) {
      out.push(n.id);
...
function BlogCommentItem({
  ...
  async function submitReply() {
    ...
    try {
      await blogMeCreateComment(postId, {
        body: replyBody.trim(),
        parentId: node.id,
      });
  ...
  async function saveEdit() {
    ...
    try {
      await blogMePatchComment(postId, node.id, { body: editBody.trim() });
  ...
  return (
    ...
    >
      <div className="flex gap-3">
        <HubAuthorAvatar
          author={node.author as HubAuthor}
          className="size-9"
          size="lg"
...
export function BlogCommentTree({
  postId,
  nodes,
}: {
  postId: string;
  nodes: BlogCommentNode[];
}) {
  const router = useRouter();
  ...
  useEffect(() => {
    ...
    const ids = collectBlogCommentIds(nodes);
    if (ids.length === 0) return;
    let cancelled = false;
    void blogMeCommentLikesBatch(ids)
      .then((r) => {
        if (cancelled) return;
  ...
  const toggleCommentLike = useCallback(
    async (commentId: string) => {
      const r = await blogMeToggleCommentLike(postId, commentId);
  ...
  const commentEngagement: CommentEngagement = {
    likeCounts,
    likedMap,
    signedIn,
    currentUserId,
  ...
  async function confirmDelete() {
    ...
    setDeleting(true);
    try {
      await blogMeDeleteComment(postId, deleteId);
