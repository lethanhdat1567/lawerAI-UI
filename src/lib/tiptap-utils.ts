...
export function isNodeTypeSelected(
  ...
  // tiptap-extension-resize-image uses node name `imageResize`, not `image`
  const types =
    nodeTypeNames.includes("image") && !nodeTypeNames.includes("imageResize")
      ? [...nodeTypeNames, "imageResize"]
      : nodeTypeNames;
  ...
  if (checkAncestorNodes) {
    ...
    for (let depth = $from.depth; depth > 0; depth--) {
      ...
      if (types.includes(ancestorNode.type.name)) {
        return true;
      }
    }
  }
...
export const handleImageUpload = async (
  ...
  const { url: path } = await authService.uploadImage(file);

  const src = resolvePublicImageUrl(path) ?? path;
  return src;
};
