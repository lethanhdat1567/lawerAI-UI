  ...
  return (
    <form onSubmit={handleSubmit(onSubmit)} className="space-y-8">
      ...
      <div className="flex flex-wrap items-end gap-4">
        <div
          className={cn(
            "flex size-24 shrink-0 items-center justify-center overflow-hidden rounded-full border border-border bg-muted",
            previewSrc ? "p-0" : "p-4",
          )}
        >
          {previewSrc ? (
      ...
      >
        <textarea
          ...
          rows={4}
          className={cn(
            "border-input placeholder:text-muted-foreground flex w-full rounded-md border bg-transparent px-3 py-2 text-sm shadow-xs outline-none transition-[color,box-shadow] disabled:pointer-events-none disabled:cursor-not-allowed disabled:opacity-50",
      ...
      <label className="flex cursor-pointer items-start gap-3 text-sm">
        ...
        <span>
          <span className="font-medium text-foreground">
            Không tham gia bảng xếp hạng đóng góp
          </span>
          <span className="mt-1 block text-muted-foreground">
            Bật nếu bạn không muốn hiển thị điểm đóng góp công khai.
          </span>
        </span>
      </label>
